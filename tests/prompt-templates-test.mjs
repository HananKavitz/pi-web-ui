...
async function main() {
	...
	check(
		`all 25 aihero skills + "new template" card shown (got ${cardTexts.length})`,
		cardTexts.length === 26,
	);
	check(
	...
	const titleVal = await page
		.locator(".template-field:nth-of-type(2) .template-field-input")
		.inputValue();
	check(
	...
	await page
		...
		.fill("冒烟模板");
	await page
		.locator(".template-field:nth-of-type(3) .template-field-input")
		.fill("一句冒烟说明");
	await page
		.locator(".template-field-prompt")
		.fill("这是一条冒烟测试提示词，第一行\n第二行");
	...
	const stored = await page.evaluate(() =>
		localStorage.getItem("pi-web-ui:prompt-templates"),
	);
	...
	const pickerCards = await page
		.locator(".template-picker .empty-template")
		.allTextContents();
	check(
