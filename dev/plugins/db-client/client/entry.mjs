...
<div class="dbx">
    ...
    async function openConn(hostId) {
      if (!state.depsOk) {
        toast(state.depsInstalling ? "驱动安装中，请稍候…" : "驱动未就绪，请先点击左侧「安装驱动」", true);
        return;
      }
    ...
    function renderGrid(host, columns, rows, kind) {
      ...
      tbl.innerHTML = "<thead><tr>" + columns.map((c) => `<th class="${canSort ? "sortable" : ""}" data-col="${esc(c)}">${esc(c)}${work?.orderBy === c ? work.dir === "desc" ? " ↓" : " ↑" : ""}</th>`).join("") + "</tr></thead>";
      if (canSort) {
      ...
      for (const row of rows) {
        ...
        tr.innerHTML = row.map((v) => v === null ? '<td class="isnull">NULL</td>' : `<td title="${esc(v)}">${esc(v)}</td>`).join("");
        tb.appendChild(tr);
      }
      tbl.appendChild(tb);
